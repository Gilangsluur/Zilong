...
def gen():
        ...
        try:
                ...
                "user-agent"                : "Mozilla/5.0 (Linux; Android 10; Redmi Note 9 Pro) AppleWebKit/537.36 (KHTML, seperti Gecko) Chrome/89.0.4389.105 Mobile Safari/537.36", # Jangan Di Ganti Ea Anjink.
...
class crackttl:
	...
	def pwlist(self):
		...
		if len(self.pw) ==0:
			...
			for i in self.fl:
				i.update({"pw":self.pw})
			print((k+"\n["+p+"•"+k+"]"+p+" Crack Started..."+k+"\n["+p+"•"+k+"]"+p+" Account [OK] Saved to : ok.txt"+k+"\n["+p+"•"+k+"]"+p+" Account [OK] Saved to : ok.txt\n"))
			ThreadPool(30).map(self.main,self.fl)
			os.remove(self.apk)
			exit()
	def main(self,fl):
